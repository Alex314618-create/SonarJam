...
async fn main() {
    ...
    loop {
        ...
        if is_key_pressed(KeyCode::L) {
            ...
            println!(
                "[ship_preview] shading = {}",
                if unlit { "unlit" } else { "lambert" }
            );
        }
        ...
        if mouse_grabbed {
            ...
            yaw += d.x * LOOK_SENS;
            pitch = (pitch - d.y * LOOK_SENS).clamp(-1.5, 1.5);
            last_mouse = Some(m);
        }
...
fn load_ship(path: &str) -> Result<Vec<Batch>, String> {
    ...
    let (doc, buffers, images) = gltf::import_slice(&patched).map_err(|e| e.to_string())?;
    ...
    for mesh in doc.meshes() {
        for prim in mesh.primitives() {
            ...
            if r.read_colors(0).is_some() {
                prim_with_vc += 1;
            }
        }
    }
    println!(
        "[ship_preview] primitives with COLOR_0 = {}/{}",
        prim_with_vc, prim_total
    );
    ...
    println!(
        "[ship_preview] built {} batches ({} base, {} emissive)",
        batches.len(),
